...
if [[ "$ARM" != "armv7l" && "$ARM" != "aarch64" ]]; then
...
if git clone --depth=1 https://github.com/DinginMastaa69/TeleFrame.git; then
...
if [[ $pmchoice =~ ^[Yy]$ ]]; then
    ...
    sudo su -c "env PATH=$PATH:/usr/bin pm2 startup systemd -u frame --hp /home/frame"
